...
def compare_N_to_time(test_image_path, save_path_folder):
    # find out how the number of points affects the time taken 
    times = []
    points = [50, 100, 200, 500, 1000, 2000, 5000, 10000]
    for N in points:
        start_time = time.time()
        colourise_process(test_image_path, save_path_folder, n=N, m=1, point_gen_method='random', sigma1=50, sigma2=100, p=0.5, delta=2.0e-10, phi_method='gaussian')
        end_time = time.time()
...
def compare_image_size_to_time(test_image_path, save_path_folder):
    ...
    for height, width in sizes:
        ...
        start_time = time.time()
        colourise_process(test_image_path, save_path_folder, n=10, m=10, point_gen_method='grid', sigma1=50, sigma2=100, p=0.5, delta=2.0e-10, phi_method='gaussian')
        end_time = time.time()
        times.append(end_time - start_time)
...
def get_ssim_score(params, test_image_path, save_path_folder, phi_method='gaussian'):
    sigma1, sigma2, p, delta = params
    colourise_process(test_image_path, save_path_folder, n=10, m=10, point_gen_method='grid', sigma1=sigma1, sigma2=sigma2, p=p, delta=delta, phi_method=phi_method)
    ssim_score = image_difference_score.ssim_difference(test_image_path, f"{save_path_folder}/colourised.png")
...
if __name__ == "__main__":
    ...
    # compare_N_to_time(test_image_path, save_path_folder)
    # compare_image_size_to_time(test_image_path, save_path_folder)
    ...
    for phi_method in phi_methods:
        ...
        result = minimize(get_ssim_score,
                          initial_guess,
                          args=(test_image_path, save_path_folder, phi_method),
                          bounds=bounds,
                          method='Nelder-Mead',
    ...
    colourise_process(test_image_path, save_path_folder, n=10, m=10, point_gen_method='grid', sigma1=best_params[0], sigma2=best_params[1], p=best_params[2], delta=best_params[3], phi_method=best_phi_method, debug=False)

    # calculate the pixelwise difference between the original and the colourised image
    pixelwise_diff = pixelwise_difference(test_image_path, f"{save_path_folder}/colourised.png")
